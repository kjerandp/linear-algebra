...
describe('Table class', () => {
  it('Should be able to initialize with values and retrieve values using get', () => {
    expect(new Table(4)).toBeInstanceOf(Table);

    const a = new Table([1, 2, 3, 4, 5, 6, 7, 8, 9]);
    const b = new Table(3, [1, 2, 3, 4, 5, 6, 7, 8, 9]);

    expect(a.getCellValue(2)).toBe(3);
    expect(a.getCellValue(3, 2)).toBeUndefined();
    expect(b.getCellValue(0, 0)).toBe(1);
    expect(b.getCellValue(1, 0)).toBe(2);
    expect(b.getCellValue(2, 0)).toBe(3);
    expect(b.getCellValue(0, 1)).toBe(4);
    expect(b.getCellValue(1, 1)).toBe(5);
    expect(b.getCellValue(2, 1)).toBe(6);
    expect(b.getCellValue(0, 2)).toBe(7);
    expect(b.getCellValue(1, 2)).toBe(8);
    expect(b.getCellValue(2, 2)).toBe(9);
    expect(b.getCellValue(7)).toBeUndefined();
    expect(a.getCellValue(10)).toBeUndefined();
    expect(b.getCellValue(1, -2)).toBeUndefined();
  ...
  it('Should be able to set values after initialization', () => {
    ...
    expect(a.setCellValue(1, 0, 3).value).toEqual([1, 3, 3, 4]);
    expect(a.cols).toBe(4);
    a.value = [1, 2, 3];
    expect(a.entries).toBe(3);

    a.value = [3, 4, 5, 6];
    expect(a.value).toEqual([3, 4, 5, 6]);
    expect(a.entries).toBe(4);
    ...
    expect(a.setCellValue(1, 1, 9).value).toEqual([1, 2, 3, 4, 9, 6, 7, 8, 9]);
    expect(a.setCellValue([1, 2, 3]).value).toEqual([1, 2, 3]);
    expect(a.setCellValue([1, 2]).value).toEqual([1, 2]);
    expect(a.setCellValue([1, 2], [3, 4, 5], 6).value).toEqual([1, 2, 3, 4, 5, 6]);
  });

  it('Can re-assign its values by using the assign method (callback)', () => {
    const a = new Table([[1, 2, 3], [4, 5, 6], [7, 8, 9]]);
    a.assign((cv, c, r, n) => {
      expect(cv).toBe(a._values[n]);
  ...
  it('Should support other datatypes than numbers', () => {
    ...
    expect(t.getCellValue(0, 1)).toBe('c');
    t.setCellValue(1, 1, 3);
